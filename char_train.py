...
"""
the train file of character-based Chinese NER model
"""
...
from ner.lw.char_ner import Char_NER as SeqModel
...
def data_initialization(data, gaz_file, train_file, dev_file, test_file):
    data.build_char_alphabet(train_file)
    data.build_char_alphabet(dev_file)
    data.build_char_alphabet(test_file)
...
def evaluate(data, model, name):
    ...
    for batch_id in range(total_batch):
        ...
        if not instance:
            continue
        batch_char, batch_charlen, batch_charrecover, batch_label, mask = batchify_with_label(instance, data.HP_gpu, True)
        tag_seq = model(batch_char, batch_charlen, mask)
...
def batchify_with_label(input_batch_list, gpu, volatile_flag=False):
    ...
    char_seq_tensor = torch.zeros((batch_size, max_seq_len)).long()
    label_seq_tensor = torch.zeros((batch_size, max_seq_len)).long()
    mask = torch.zeros((batch_size, max_seq_len)).byte()

    for idx, (seq, label, seqlen) in enumerate(zip(chars, labels, char_seq_lengths)):
    ...
    if gpu:
        ...
        mask = mask.cuda()
        char_seq_lengths = char_seq_lengths
        char_seq_recover = char_seq_recover.cuda()

    return char_seq_tensor, char_seq_lengths, char_seq_recover, label_seq_tensor, mask
...
def train(data, save_model_dir, seg=True):
    ...
    for idx in range(epoch, data.HP_iteration):
        ...
        for batch_id in range(total_batch):
            ...
            if not instance:
                continue
            batch_char, batch_charlen, batch_charrecover, batch_label, mask = batchify_with_label(instance, data.HP_gpu)
            # print "gaz_list:",gaz_list
            # exit(0)
            instance_count += (end - start)
            loss, tag_seq = model.neg_log_likelihood_loss(batch_char, batch_charlen, batch_label, mask)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train', default="data/test/train.char.bmes")
    parser.add_argument('--dev', default="data/test/dev.char.bmes")
    parser.add_argument('--test', default="data/test/test.char.bmes")
    ...
    if status == 'train':
        ...
        data.generate_instance_char(train_file, "train")
        data.generate_instance_char(dev_file, "dev")
        data.generate_instance_char(test_file, "test")

        print(data.char_alphabet_size)
        print(data.label_alphabet_size)
