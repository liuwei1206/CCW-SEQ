...
"""
the train file of word-based Chinese NER model, the input file is  auto-seg word-based dataset
"""
...
from ner.functions.word_metric import get_auto_word_ner_measure
from ner.lw.word_ner import Word_NER as SeqModel
from ner.utils.cw_data import Data
...
def evaluate(data, model, name):
    if name == "train":
        ...
        char_labels = data.train_char_labels
    elif name == "dev":
        instances = data.dev_Ids
        instance_texts = data.dev_texts
        char_labels = data.dev_char_labels
    elif name == 'test':
        instances = data.test_Ids
        instance_texts = data.test_texts
        char_labels = data.test_char_labels
    elif name == 'raw':
    ...
    for batch_id in range(total_batch):
        ...
        if not instance:
            continue
        batch_word, batch_wordlen, batch_wordrecover, batch_label, mask = batchify_with_label(instance, data.HP_gpu, True)
    ...
    acc, p, r, f = get_auto_word_ner_measure(char_labels, gold_results, [instance_texts, pred_results])
...
def train(data, save_model_dir, seg=True):
    ...
    for idx in range(epoch, data.HP_iteration):
        ...
        for batch_id in range(total_batch):
            ...
            # a origin batch instance data
            instance = data.train_Ids[start:end]
            if not instance:
                continue
            batch_word, batch_wordlen, batch_wordrecover, batch_label, mask = batchify_with_label(instance, data.HP_gpu)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--savemodel', default="data/model/test")
    parser.add_argument('--savedset', help='Dir of saved data setting', default="data/save.dset")
    parser.add_argument('--train', default="data/test/auto-word/train.word.bmes")
    parser.add_argument('--dev', default="data/test/auto-word/dev.word.bmes")
    parser.add_argument('--test', default="data/test/auto-word/test.word.bmes")
    parser.add_argument('--c_train', default='data/test/char/train.char.bmes')
    parser.add_argument('--c_dev', default='data/test/char/dev.char.bmes')
    parser.add_argument('--c_test', default='data/test/char/test.char.bmes')
    ...
    train_char_file = args.c_train
    test_char_file = args.c_test
    dev_char_file = args.c_dev
    ...
    if status == 'train':
        ...
        data.generate_instance_word(train_file, "train")
        data.generate_instance_word(dev_file, "dev")
        data.generate_instance_word(test_file, "test")
        data.get_char_label(train_char_file, "train")
        data.get_char_label(dev_char_file, "dev")
        data.get_char_label(test_char_file, "test")

        print('test number: ', len(data.test_texts))

        # for embedding file
        if data is None:
            print('ok')
        data.build_word_pretrain_emb()
        end_time = time.time()
        print('preprocessing time is: ', (end_time - start_time), 's')
