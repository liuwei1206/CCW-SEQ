...
since big embedding is 200 dim, which is different from origin 50, so we add a project 
to project 200 into 50
"""
import numpy as np
import torch
...
class Big_CW_NER(torch.nn.Module):
    def __init__(self, data, type=1):
        ...
        self.b_proj_drop = nn.Dropout(data.HP_dropout)
        self.emb_project = nn.Linear(data.gaz_emb_dim, data.word_emb_dim * 2)

        self.lstm = Gaz_BiLSTM(data, data.word_emb_dim * 3, data.HP_hidden_dim)
        ...
        if self.gpu:
            ...
            self.emb_project = self.emb_project.cuda()
            self.b_proj_drop = self.b_proj_drop.cuda()
    ...
    def neg_log_likelihood_loss(self, gaz_list, reverse_gaz_list, word_inputs, word_seq_lengths, batch_label, mask):
        ...
        # dropout before and after project?
        gaz_embs = self.b_proj_drop(gaz_embs)
        reverse_gaz_embs = self.b_proj_drop(reverse_gaz_embs)

        gaz_embs = self.emb_project(gaz_embs)
        reverse_gaz_embs = self.emb_project(reverse_gaz_embs)
